...
#include <iauv_kinematic_control/TaskState.h>
...
public:
    ...
    double error = 0;
    double x, y = 0;
    ros::Publisher tp_auv_pos;
    ros::Publisher tp_auv_yaw;
    ros::Subscriber tp_auv_feed;
    ...
    PathManager(ros::NodeHandle nh) : as_(nh, "path_manager", false)
    {
        ...
        tp_auv_pos = nh.advertise<geometry_msgs::PoseStamped>("/girona1000/tp_controller/tasks/auv_position/target", 1);
        tp_auv_yaw = nh.advertise<geometry_msgs::PoseStamped>("/girona1000/tp_controller/tasks/auv_orientation/target", 1);
        tp_auv_feed = nh.subscribe("/girona1000/tp_controller/tasks/auv_position/state", 10, &PathManager::feedbackCb, this);
    }
    ...
    void feedbackCb(const iauv_kinematic_control::TaskStateConstPtr &feedMsg)
    {
        feedMsg->error;
        Eigen::Vector3d v(feedMsg->error.data());
        error = v.norm();
    }
            ...
            else
            {
                ...
                if (error < 0.2)
                {
                    ...
                    if (counter < path_.poses.size())
                    {
                        ...
                        poseS.header.frame_id = "world_ned";
                        poseS.header.stamp = ros::Time::now();
                        poseS.pose = path_.poses.at(counter).pose;

                        // if (counter == 0)
                        // {
                        //     x = path_.poses.at(counter).pose.position.x;
                        //     y = path_.poses.at(counter).pose.position.y;
                        // }
                        // else
                        // {
                        //     x = path_.poses.at(counter).pose.position.x - path_.poses.at(counter - 1).pose.position.x;
                        //     y = path_.poses.at(counter).pose.position.y - path_.poses.at(counter - 1).pose.position.y;
                        // }
                        // double yaw = atan2(y, x);

                        // Eigen::Quaterniond q(Eigen::AngleAxisd(yaw, Eigen::Vector3d::UnitZ()));
                        // poseS.pose.orientation = tf2::toMsg(q);
                        // poseS.pose.orientation = path_.poses.at(counter).pose.orientation;

                        tp_auv_pos.publish(poseS);
                        tp_auv_yaw.publish(poseS);
